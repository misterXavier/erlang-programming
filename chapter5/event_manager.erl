...
-export([add_handler/3, delete_handler/2, swap_handler/3, get_data/2, send_event/2]).
-export([init/1]).
...
handle_msg({swap_handler, OldHandler, NewHandler}, LoopData) ->
  case handle_msg({delete_handler, OldHandler}, LoopData) of
    {{data, HandlerState}, NewLoopData} -> handle_msg({add_handler, NewHandler, HandlerState}, NewLoopData);
    {Error, NewLoopData} -> {Error, NewLoopData}
  end;
