...
start(Times, Processes, Msg, First) when First == self() ->
  io:format("Node ~w received (~B p ~B). * First~n", [self(), Times, Processes]),
  Next = spawn(ring, start, [Times, Processes-1, Msg, First]),
  Next ! {message, Times, Msg},
  serve_requests(Next);

start(Times, Processes, _Msg, First) when Processes == 1 ->
